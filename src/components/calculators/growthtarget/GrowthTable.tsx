    ...
    <Box>
      <Alert severity="info" sx={{ mb: 3 }}>
        <Typography variant="subtitle1">
          To reach your target, you need an annual growth rate of:{" "}
          <strong>{requiredRate.toFixed(2)}%</strong>
        </Typography>
      </Alert>
        ...
        <Table aria-label="growth projection table">
          ...
          <TableBody>
            {yearlyData.map((row, index) => {
              const prevBalance =
                index > 0 ? yearlyData[index - 1].balance : row.balance;
