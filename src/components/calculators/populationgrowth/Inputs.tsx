...
import { usePopulationGrowth, PRESETS, ScenarioState, getFlagEmoji } from "./PopulationGrowthContext";
...
const Inputs: React.FC = () => {
  ...
  const renderScenarioInputs = (
    ...
    return (
      <Paper elevation={2} sx={{ p: 2.5, borderRadius: 2, borderTop: `4px solid ${primaryColor}` }}>
        <Stack spacing={2.5}>
          ...
          <FormControl size="small" fullWidth>
            ...
            <Select
              ...
              onChange={(e) => handlePresetChange(e.target.value as string, setScenario)}
              renderValue={(value) => `${getFlagEmoji(value as string)} ${PRESETS[value as string]?.name || value}`}
            >
          ...
          <Box>
            <Typography variant="body2" sx={{ fontWeight: "bold", mb: 0.5 }}>
              Initial Population: {scenario.initialPopulation >= 1000000000
            ...
            <Slider
              ...
              onChange={(_, val) => handleCustomChange("initialPopulation", val as number, setScenario)}
              size="small"
              valueLabelDisplay="auto"
              valueLabelFormat={(val) => val >= 1000000000 ? `${(val / 1000000000).toFixed(1)}B` : `${(val / 1000000).toFixed(0)}M`}
          ...
          <Box>
            ...
            <Slider
              ...
              onChange={(_, val) => handleCustomChange("fertility", val as number, setScenario)}
              size="small"
              valueLabelDisplay="auto"
          ...
          <Box>
            ...
            <Slider
              ...
              onChange={(_, val) => handleCustomChange("lifeExpectancy", val as number, setScenario)}
              size="small"
              valueLabelDisplay="auto"
          ...
          <Box>
            <Typography variant="body2" sx={{ fontWeight: "bold", mb: 0.5 }}>
              Annual Net Migration: {scenario.netMigration >= 0 ? "+" : ""}{scenario.netMigration.toLocaleString()} people
            </Typography>
            <Slider
              min={-1000000}
              max={2000000}
              ...
              onChange={(_, val) => handleCustomChange("netMigration", val as number, setScenario)}
              size="small"
              valueLabelDisplay="auto"
              valueLabelFormat={(val) => `${val >= 0 ? "+" : ""}${(val / 1000).toFixed(0)}k`}
              sx={{ color: primaryColor }}
            />
          </Box>
        </Stack>
